...
#     """Use ChatGPT for song list; search YouTube ourselves for playable videos."""
#     try:
...
#         # Build user prompt (with exclusions by title+artist)
#         if exclude_songs:
#             exclude_list = [f"{song.get('title','')} by {song.get('artist','')}" for song in exclude_songs]
...
#             # Fallback: one broad search using the prompt
...
#         seen_ids = set()
#         seen_titles = set()  # avoid duplicate songs by title+artist

#         for song_info in song_data:
#             # Dedup on (title, artist)
#             key = (song_info.get("title","").lower(), song_info.get("artist","").lower())
#             if key in seen_titles:
#                 continue
#             seen_titles.add(key)

#             vid = _search_youtube_by_song(song_info, limit=12)
...
<style>
    ...
    .video-card { border: 2px solid #ddd; border-radius: 10px; padding: 1rem; margin: 1rem 0; background-color: #f8f9fa; text-align: center; }
    .prompt-input { background-color: #f0f2f6; border-radius: 10px; padding: 1rem; margin: 1rem 0; text-align: center; }
    .centered-text { text-align: center; }
    .element-container input { text-align: center !important; }
</style>
""",
    unsafe_allow_html=True,
)
...
def extract_youtube_links(text: str):
    patterns = [
        r"(?:https?://)?(?:www\.)?youtube\.com/watch\?v=([a-zA-Z0-9_-]+)",
        r"(?:https?://)?(?:www\.)?youtu\.be/([a-zA-Z0-9_-]+)",
        r"(?:https?://)?(?:www\.)?youtube\.com/embed/([a-zA-Z0-9_-]+)",
    ...
    seen = set()
    uniq = [v for v in video_ids if not (v in seen or seen.add(v))]
    return uniq


def get_video_info(video_id: str):
    """Check availability quickly via YouTube oEmbed."""
    try:
        oembed_url = (
        ...
        if response.status_code == 200:
            data = response.json()
            return {"title": data.get("title", "Unknown Title"), "video_id": video_id, "available": True}
        else:
            return {"title": f"Video {video_id[:8]}...", "video_id": video_id, "available": False}
    except Exception:
        return {"title": f"Video {video_id[:8]}...", "video_id": video_id, "available": False}


# ---------- New search-based approach (ignore GPT links) ----------

def _normalize_key(song):
    return (
...
def _build_search_query(song_info):
    parts = [song_info.get("title", ""), song_info.get("artist", "")]
    src = song_info.get("source", "").strip()
    if src and len(src) <= 40:
        parts.append(src)
    parts.append("official audio")  # bias to playable versions
    return " ".join(p for p in parts if p).strip()


def _is_bad_candidate(title: str) -> bool:
    t = title.lower()
    bad = ["cover", "karaoke", "instrumental", "remix", "sped up", "slowed", "nightcore"]
...
def _score_candidate(item, song_info) -> int:
    ...
    if "vevo" in channel:
        score += 3
    if " - topic" in channel or (artist and f"{artist} - topic" in channel):
        score += 4
    if artist and artist in channel:
        score += 2
    if "official" in title:
...
def search_youtube_candidates(song_info, limit=15):
    q1 = _build_search_query(song_info)
    results = VideosSearch(q1, limit=limit).result().get("result", [])
    if not results:
        q2 = f"{song_info.get('title','')} {song_info.get('artist','')}"
        results = VideosSearch(q2, limit=limit).result().get("result", [])
        if not results:
            return []
...
def use_next_alternate_for_current_song():
    idx = st.session_state.get("current_song_index", 0)
    cands = st.session_state.get("song_candidates", {}).get(idx, [])
    if not cands:
        return
    current = st.session_state.get("selected_video")
    if current in cands:
        pos = cands.index(current)
        nxt = cands[(pos + 1) % len(cands)]
    else:
        nxt = cands[0]
    st.session_state.selected_video = nxt
    st.session_state.auto_play = True
...
def get_youtube_videos_with_chatgpt(prompt, exclude_songs=None):
    ...
    try:
        ...
        for song_info in song_data:
            ...
            candidates = search_youtube_candidates(song_info, limit=15)
            if not candidates:
                continue

            chosen = None
            for vid in candidates:
                if get_video_info(vid).get("available", False):
                    chosen = vid
                    break
...
st.markdown('<div class="main-header">🎵 YouTube Video Game 🎵</div>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.markdown('<div class="centered-text"><h3>💡 Example Prompts</h3></div>', unsafe_allow_html=True)
    st.markdown(
        '<div class="centered-text">- "songs from movies"<br>- "80s pop hits"<br>- "classic rock songs"<br>- "disney movie songs"<br>- "video game music"</div>',
        unsafe_allow_html=True,
    )
...
with col_left:
    st.markdown('<div class="sub-header">🎯 Enter Your Video Prompt</div>', unsafe_allow_html=True)
    with st.container():
        st.markdown('<div class="prompt-input">', unsafe_allow_html=True)
        prompt = st.text_input(
            "What kind of videos are you looking for?",
                ...
                with st.spinner("🎵 Generating your video collection..."):
                    ...
                    if video_ids:
                        ...
                        for vid in video_ids:
                            info = get_video_info(vid)
                            st.session_state.videos.append(info)
                    else:
                        st.error("❌ No videos found. Please try a different prompt.")
...
if "videos" in st.session_state and st.session_state.videos:
    st.markdown(
        '<h2 class="sub-header" style="text-align: center;">🎵 Song Guessing Game</h2>',
        unsafe_allow_html=True,
    )
    st.markdown(
        "<div style=\"text-align: center;\"><strong>Listen to each song and try to guess what it is and where it's from!</strong></div>",
        unsafe_allow_html=True,
    )
    ...
    if current_index < total_songs:
        current_video = st.session_state.videos[current_index]
        st.markdown(
            f'<div style="text-align: center;"><strong>Song {current_index + 1} of {total_songs}</strong></div>',
            unsafe_allow_html=True,
        )
        if not current_video.get("available", True):
            st.markdown('<div style="text-align: center;">', unsafe_allow_html=True)
        ...
        with c2:
            if st.button("⏭️ Next Song", key="next_song", use_container_width=True):
                if current_index < total_songs - 1:
                    st.session_state.current_song_index = current_index + 1
                    st.session_state.selected_video = st.session_state.videos[current_index + 1][
                        "video_id"
                    ]
                    st.session_state.current_song_number = current_index + 2
                    st.session_state.auto_play = True
        ...
        with c3:
            if st.button("⏮️ Previous Song", key="prev_song", use_container_width=True):
                if current_index > 0:
                    ...
                    st.rerun()
        with c4:
            if st.button("⚠️ Try alternate link", key="alt_link", use_container_width=True):
                use_next_alternate_for_current_song()
                st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)
...
# Spacer to push player
st.markdown("<br>" * 12, unsafe_allow_html=True)

if "selected_video" in st.session_state:
    song_number = st.session_state.get("current_song_number", "?")
    st.markdown(
        f'<h2 class="sub-header" style="text-align: center;">🎵 Now Playing: Song #{song_number}</h2>',
        unsafe_allow_html=True,
    )
    ...
    if "videos" in st.session_state and current_index < len(st.session_state.videos):
        ...
        if not current_video.get("available", True):
            st.markdown('<div style="text-align: center;">', unsafe_allow_html=True)
            st.warning(
                "⚠️ This video may not be available in your region. Click ‘Try alternate link’."
            )
            st.markdown("</div>", unsafe_allow_html=True)
    ...
    st.markdown('<h3 style="color: #ff6b6b;">🎬 Open in YouTube</h3>', unsafe_allow_html=True)
    st.markdown(
        "<p><strong>If the video doesn't play, try opening it directly on YouTube:</strong></p>",
        unsafe_allow_html=True,
    )
    st.markdown(
        '<a href="https://www.youtube.com/watch?v='
        + video_id
        + '" target="_blank" style="background-color: #ff0000; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; font-weight: bold;">▶️ Open in YouTube</a>',
        unsafe_allow_html=True,
    )
    st.markdown("</div>", unsafe_allow_html=True)

    if st.button("⚠️ Not working? Try alternate link", use_container_width=True):
        use_next_alternate_for_current_song()
        st.rerun()
...
#             url = song_info.get("link") or ""
#             ids = extract_youtube_links(url)
#             if not ids:
#                 continue
#             vid = ids[0]
#             if vid in seen_ids:
